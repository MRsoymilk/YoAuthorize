...
TEST(P2PNodeHttpTest, StatusAndNodesApi) {
  std::string url = "tls+tcp://127.0.0.1:5555";
  std::string broadcast_url = "tls+tcp://127.0.0.1:5557";
  ya::YaCommunicate::P2PNode node("test_node", url, broadcast_url);
  node.start_http_server(8000);
  std::string res_status = exec("curl -s http://127.0.0.1:8000/status");
  std::cout << "/status: " << res_status << std::endl;
  std::string res_nodes = exec("curl -s http://127.0.0.1:8000/nodes");
...
TEST(YaCommunicateTest, TLSCommunicationTest) {
  try {
    std::string broadcast_url = "tls+tcp://127.0.0.1:5557";
    ya::YaCommunicate::P2PNode node("node", "tls+tcp://127.0.0.1:5555",
                                    broadcast_url);
